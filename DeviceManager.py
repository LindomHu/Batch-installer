...
from InstalliOS import BatchInstalliOS
...
class DeviceManager():
    def __init__(self):
        ...
        self.install_iOS_item_list = []
        self.uninstall_iOS_item_list = []
    ...
    def item_iOS_device_return(self):
        self.device_iOS_list = StringVar()
        return self.device_iOS_list.get()
    ...
    def refresh_tid(self):
        ...
        old_iOS_list = self.device_iOS_list  # 原来连接的设备
        new_iOS_list = self.get_device_iOS_list()  # ==>adb device
        # old-new 的差集用old_new表示
        old_iOS_new = set(old_iOS_list) - set(new_iOS_list)
        for iOS_item in (self.install_iOS_item_list):
            if iOS_item.device in list(old_iOS_new):
                iOS_item.destroy()

        # new-old 的差集用new_old_iOS表示
        new_old_iOS = set(new_iOS_list) - set(old_iOS_list)  # 原来的设备 a  新插入一台b   ==b
        start_iOS_draw_index = len(set(old_iOS_list) & set(new_iOS_list))  # a—>len
        old_iOS_new_jiao = set(old_iOS_list ) & set(new_iOS_list)
        cur_iOS = list(old_iOS_new_jiao) + list(new_old_iOS)
        result = set(new_iOS_list ).issubset(set(cur_iOS))
        if result == True:
            for iOS_item in (self.install_iOS_item_list):
                if iOS_item.device in list(cur_iOS):
                    iOS_item.destroy()
        self.draw_installer_iOS_item(cur_iOS,start_iOS_draw_index + 1)  # ((b),1)
        self.device_iOS_list = new_iOS_list
    ...
    # 获取tid连接上的iOS
    def get_device_iOS_list(*args, **kwargs):  # 得到设备列表
        os.system("tidevice list --json")
        device_iOS_list1 = os.popen("tidevice list --json")
        device_iOS_list2 = json.loads(device_iOS_list1.read())
        device_iOS_list = (device_iOS_list2[0]["udid"])
        print("devcelist:", device_iOS_list)
        print(type(device_iOS_list))
        # device_list1 = [sub.split('\n')[0] for sub in res[1:-1]]
        # device_list = [sub.replace('\t','-') for sub in device_list1[:]]
    ...
    def draw_installer_item(self, device_list, old_device_num=0):
        for index, device in enumerate(device_list):
            ...
            install_item = BatchInstallApk(self.root, device, start_row, self)
            self.install_item_list.append(install_item)
            self.root.grid_columnconfigure((0, 3, 5), minsize=20)
            return index

    def draw_installer_iOS_item(self, device_iOS_list, old_device_num=0):
        for index_iOS, device_iOS in enumerate(device_iOS_list):
            start_iOS_row = index_iOS + old_device_num + 2
            install_iOS_item = BatchInstalliOS(self.root, device_iOS, start_iOS_row, self)
            self.install_iOS_item_list.append(install_iOS_item )
            self.root.grid_columnconfigure((0, 3, 5), minsize=20)
    ...
    def mainloop(self):
        ...
        self.draw_installer_iOS_item(self.device_iOS_list)
