...
class DeviceManager():
    ...
    def get_device_iOS_list(*args, **kwargs):  # 得到设备列表
        ...
        res_list1 = os.popen("tidevice list").readlines()
        res_list2 = [sub.split(" ")[0] for sub in res_list1[:]]
        device_iOS_list = [sub.split(",")[0] for sub in res_list2[0:-1]]
        # device_list1 = [sub.split('\n')[0] for sub in res[1:-1]]
        # device_list = [sub.replace('\t','-') for sub in device_list1[:]]
